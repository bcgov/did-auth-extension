function onRadioClick1() {
	...
	browser.storage.local.set({ 'didnr': 0 });
}

function onRadioClick2() {

	browser.storage.local.set({ 'didnr': 1 });
}

function onRadioClick3() {

	browser.storage.local.set({ 'didnr': 2 });
}

function onCheckBoxClick1() {

	var vc1 = document.getElementById("vc1");
	browser.storage.local.set({ 'vc1': vc1.checked });
}

function onCheckboxClick2() {

	var vc2 = document.getElementById("vc2");
	browser.storage.local.set({ 'vc2': vc2.checked });
}
...
function init() {
	...
	browser.storage.local.get('didnr').then(function(value) {
	...
	browser.storage.local.get('vc1').then(function(value) {

		if (value.vc1 === true) vc1.checked = true; else vc2.checked = false;
	});

	browser.storage.local.get('vc2').then(function(value) {
